...
def _warn_invalid_github_token_once() -> None:
    ...
    then ignore it so downstream behaves unauthenticated.
    """
    ...
    if not looks_valid:
        ...
        # Treat as unset so any GitHub calls proceed unauthenticated instead of half-authing.
        os.environ["GITHUB_TOKEN_INVALID"] = tok  # optional breadcrumb for debugging
...
def main() -> int:
    args = parse_args()
    _ = get_logger()  # initialize logging (handles LOG_FILE / LOG_FILE_PATH fallback)
    try:
        _warn_invalid_github_token_once()  # <-- this now also unsets a bad token
